...
def main():
    ...
    soma = find_soma_section(sections)
        ...
        # ========== 手动添加人工 AIS (长度 50 µm, 与 soma 末端 z 平面对齐) ==========
    # 获取 soma 末端的 3D 坐标（使用 section 的方法）
    n3d = int(soma.pt3dcount())  # 正确写法
    if n3d > 0:
        x_end = soma.pt3dgetx(n3d-1)
        y_end = soma.pt3dgety(n3d-1)
        z_end = soma.pt3dgetz(n3d-1)
        print(f"Soma end at: ({x_end:.1f}, {y_end:.1f}, {z_end:.1f})")
    else:
        x_end, y_end, z_end = 0.0, 0.0, 0.0
        print("Warning: Soma has no 3D points, using (0,0,0)")
    ...
    # 添加 3D 点：起点为 soma 末端，沿 x 轴正向延伸 50 µm（保持 z 不变）
    ais.pt3dadd(x_end, y_end, z_end, ais.diam)          # 起点
    ais.pt3dadd(x_end + 50.0, y_end, z_end, ais.diam)  # 终点
    ...
    print(f"Added AIS: L={ais.L} um, diam={ais.diam} um, gnabar={ais.gnabar_hh}")
    # ========== AIS 添加结束 ==========
